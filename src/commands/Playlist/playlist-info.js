...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
        ...
        const embed = new EmbedBuilder()
            .setAuthor({name: `| ${message.author.username}'s Playlist Info`, iconURL: message.member.displayAvatarURL({dynamic:true})})
            .setColor(client.embedColor)
            .setDescription(`**Name:** \`${pname}\` \n**Tracks:** \`${plist}\`\n\n${pages[page]}`)
        if (pages.length <= 1) {
            ...
            collector.on("collect", async (b) => {
                ...
                if (b.customId === "Previous") {
                    ...
                    embed.setDescription(`**Name:** \`${pname}\` \n**Tracks:** \`${plist}\`\n\n${pages[page]}`);
