...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
        ...
        if (!data.length) {
            return message.reply({ embeds: [new EmbedBuilder().setColor(client.embedColor).setDescription(`You Haven't Created Any Playlist To Show`)] })
        }
        if (!args[0]) {
            let list = data.map((x, i) => `\`${++i}\` - **${x.PlaylistName}** \`${x.Playlist.length} Song(s)\` - <t:${x.CreatedOn}:R>`);
