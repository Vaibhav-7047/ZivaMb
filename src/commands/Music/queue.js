  ...
  execute: async (message, args, client, prefix) => {
    ...
    if (player.queue.length === "0" || !player.queue.length) {
      const embed = new EmbedBuilder()
        .setColor(client.embedColor)
        .setDescription(
          `Now Playing [${player.queue.current.title}](https://discord.gg/presto) - \`[${convertTime(queue.current.duration)}]\` - [${
      ...
      if (player.queue.size < 11) {
        const embed = new EmbedBuilder()
          .setColor(client.embedColor)
          .setDescription(
            `**Now Playing**\n[${player.queue.current.title}](https://discord.gg/presto) - \`[${convertTime(queue.current.duration)}]\` - [${
              player.queue.current.requester
            }]\n\n**Queued Songs**\n${pages[page]}`
          )
          .setTimestamp()
