...
module.exports = {
    ...
    if(!player) return await interaction.editReply({
        ...
        if(!player.queue.size || player.queue.size === 0) {
            ...
            const embed = new EmbedBuilder().setColor(client.embedColor).setDescription(`Now Playing [${player.queue.current.title}](https://discord.gg/presto) - \`[ ${convertTime(player.position)} / ${convertTime(player.queue.current.duration)} ]\` - [${player.queue.current.requester}]`);
            ...
            if(player.queue.size < 10 || player.queue.totalSize < 10) {
                ...
                const embed2 = new EmbedBuilder().setTitle(`${interaction.guild.name} Server Queue`).setColor(client.embedColor).setDescription(`**Now Playing**\n[${player.queue.current.title}](https://discord.gg/presto) - \`[ ${convertTime(player.position)} / ${convertTime(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter({text: `Page ${page + 1}/${pages.length}`, iconURL: interaction.user.displayAvatarURL({ dynamic: true }),}).setTimestamp()
                ...
                collector.on("collect", async (button) => {
                    if(button.customId === "queue_cmd_but_1_app") {
                        ...
                        const embed4 = new EmbedBuilder().setColor(client.embedColor).setDescription(`**Now Playing**\n[${player.queue.current.title}](https://discord.gg/presto) - \`[ ${convertTime(player.position)} / ${convertTime(player.queue.current.duration)} ]\` - [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter({text: `Page ${page + 1}/${pages.length}`, iconURL: button.user.displayAvatarURL({ dynamic: true })}).setTimestamp()
